...
INSERT INTO MESSAGE
(MESSAGE_NO, MESSAGE_CONTENT, SEND_DATE, SEND_MEMBER, RECEIVED_MEMBER)
VALUES(SEQ_MESSAGE_NO.NEXTVAL, '안녕~난 호나쓰~', SYSDATE, 2, 50)
;
-- 메세지 삽입
INSERT INTO MESSAGE
(MESSAGE_NO, MESSAGE_CONTENT, SEND_DATE, SEND_MEMBER, RECEIVED_MEMBER)
VALUES(SEQ_MESSAGE_NO.NEXTVAL, '햄스터 정모 제발 부탁드려요....프로필 사진 개인적으로 써도 될까요?', SYSDATE, 2, 50)
;
...
SELECT MESSAGE_NO, MESSAGE_CONTENT, SEND_DATE, 
		SEND_MEMBER, RECEIVED_MEMBER, MESSAGE_DEL_FL, MESSAGE_SEND_DEL_FL
FROM MESSAGE;
...
SELECT NOTICE_NO, NOTICE_COMENT, ELAPSED_TIME, MEMBER_NO, SEND_NO, SEND_PROFILE
FROM NOTICE;

		SELECT NOTICE_NO, NOTICE_COMENT, ELAPSED_TIME, MEMBER_NO,
			   MEMBER_NICKNAME , SEND_NO, SEND_PROFILE
		FROM NOTICE
		JOIN "MEMBER" USING (MEMBER_NO)
		WHERE MEMBER_NO = 1
		ORDER BY 1 DESC
...
JOIN "MEMBER_PROFILE" PF ON F.MEMBER_NO = PF.MEMBER_NO
WHERE F.MEMBER_NO = 2
ORDER BY F.FOLLOW_NO
;


SELECT F.MEMBER_NO, F.FOLLOWER_NO, M2.MEMBER_NICKNAME AS FOLLOWER_NICKNAME, PF.PROFILE_IMG AS FOLLOW_PROFILE_IMG
FROM FOLLOW F
JOIN "MEMBER" M1 ON F.MEMBER_NO = M1.MEMBER_NO
JOIN "MEMBER" M2 ON F.FOLLOWER_NO = M2.MEMBER_NO
JOIN "MEMBER_PROFILE" PF ON F.MEMBER_NO = PF.MEMBER_NO
...
INSERT INTO FOLLOW
(FOLLOW_NO, MEMBER_NO, FOLLOWER_NO)
VALUES(SEQ_FOLLOW_NO.NEXTVAL, 2, 47)
;
COMMIT;